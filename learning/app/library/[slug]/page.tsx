...
export default function LibraryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`pcg-mastery-${libraryId}`);
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (masteredConcepts.size > 0) {
      ...
      localStorage.setItem(`pcg-mastery-${libraryId}`, JSON.stringify(obj));
    }
  }, [masteredConcepts, libraryId]);

  // Show loading state
  if (loading || !library || !conceptGraphData) {
    return (
